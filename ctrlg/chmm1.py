...
class HMM(nn.Module, PyTorchModelHubMixin):
    def __init__(self, vocab_size: int, eos_token_id: int, nclones_per_state: list):
        super().__init__()

        assert len(nclones_per_state)==vocab_size, "One value per state for num clones"

        hidden_states= sum(nclones_per_state)
        ...
        for i in range(vocab_size):
            for _ in range(nclones_per_state[i]):
                beta.append([count, i])
                count+=1
