...
def update_flows(alpha, beta, gamma, suffixes, idx2cluster,
    ...
    for idx in tqdm(range(0, len(suffixes))):
        ...
        suffix_offset_next_next = (suffix_offset[0], suffix_offset[1] + 2)
        ...
        # w (t+2)
        w = None
        if suffix_offset_next_next in offset2index:
            w = idx2cluster[offset2index[suffix_offset_next_next]]
        else:
            w = hidden_states - 1  # falls back to <eos> state
    ...
    for i in range(hidden_states):
        alpha[i, hidden_states - 1, hidden_states - 1] = 1.0
...
def write_params(alpha_flow, beta_flow, gamma_flow, pseudocount,
    ...
    gamma_flow += pseudocount / (gamma_flow.shape[0] * gamma_flow.shape[1])
...
def main():
    ...
    # alpha is 3D [H, H, H], gamma is 2D [H, H]
    alpha = torch.zeros(hidden_states, hidden_states, hidden_states)
    beta = torch.zeros(hidden_states, vocab_size)
    gamma = torch.zeros(hidden_states, hidden_states)
