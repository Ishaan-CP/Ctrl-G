...
def train_hmm(rank, world_size,
    ...
    hmm_model = SOHMM.from_pretrained(f'{model_path}/checkpoint-{checkpoint}', map_location='cpu').to(device)
    ...
    for step_count, step_size in em_schedule:
        for step_idx in range(0, step_count):
            ...
            alpha_flow = torch.zeros(hidden_states, hidden_states, hidden_states, device=device)
            beta_flow = torch.zeros(vocab_size + 1, hidden_states, device=device) # (emissions only depend on z_t)
            gamma_flow = torch.zeros(hidden_states, hidden_states, device=device)
            ...
            with torch.no_grad():
                ...
                gamma_flow.div_(torch.sum(gamma_flow)).log_()
